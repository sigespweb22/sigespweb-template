...
namespace Sigesp.WebUI.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            var currenteUserId = _userResolverService.GetUserId();
            var contaUsuarioViewModel = new ContaUsuarioViewModel();
            var contaUsuario = new ContaUsuario();
            var apiKey = new Guid();
            bool isServidorEstado = false;

            if (currenteUserId != null 
                && currenteUserId != Guid.Empty)
            {
                var applicationUserDB = _userManager.Users
                                            .Include(x => x.Tenant)
                                            .Include(x => x.ServidorEstado)
                                            .Where(x => x.Id == currenteUserId.ToString())
                                            .FirstOrDefault();
                                            
                if (applicationUserDB != null)
                {
                    if (applicationUserDB.ServidorEstado != null)
                    {
                        isServidorEstado = true;
                    }
                } else {
                    applicationUserDB = _userManager.Users
                                            .Include(x => x.Tenant)
                                            .Where(x => x.Id == currenteUserId.ToString())
                                            .FirstOrDefault();
                }

                apiKey = applicationUserDB.Tenant.ApiKey;
                contaUsuario = _contaUsuarioRepository
                                                .GetByUserId(applicationUserDB.Id);
            ...
            if (isServidorEstado)
            {
                return RedirectToAction(actionName: "List", 
                                            controllerName: "ServidorEstadoReforco");
            }
            else
            {
                return RedirectToAction(actionName: "Todos", 
                                            controllerName: "Mural");
                // return RedirectToAction(actionName: "Educacao", 
