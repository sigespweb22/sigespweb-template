using System;
using System.Linq;
using System.Collections.Generic;
using System.Threading.Tasks;
using Microsoft.AspNetCore.Identity;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using Sigesp.WebUI.Extensions;
using Sigesp.Domain.Models;
using Sigesp.Application.Interfaces;
using Sigesp.Domain.Interfaces;
using Sigesp.Application.ViewModels;
using Sigesp.Application.ViewModels.Cards;
using Microsoft.AspNetCore.Authorization;
using Sigesp.Infra.CrossCutting.Identity.Services;
using AutoMapper;
using Sigesp.Domain.Enums;
using Sigesp.Application.ViewModels.Selects;

namespace Sigesp.WebUI.Controllers
{
    [Authorize(Roles = "Usuarios_Todos, Master, Usuarios_Perfil")]
    public class UsuarioController : Controller
    {
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly IContaUsuarioAppService _contaUsuarioAppService;
        private readonly RoleManager<ApplicationRole> _rolesManager;
        private readonly UserResolverService _userResolverService;
        private readonly IContaUsuarioRepository _contaUsuarioRepository;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork _unitOfWork;

        public UsuarioController(UserManager<ApplicationUser> userManager,
                                 RoleManager<ApplicationRole> rolesManager,
                                 UserResolverService userResolverService,
                                 IContaUsuarioRepository contaUsuarioRepository,
                                 IMapper mapper,
                                 IContaUsuarioAppService contaUsuarioAppService,
                                 IUnitOfWork unitOfWork)
        {
            _userManager = userManager;
            _rolesManager = rolesManager;
            _userResolverService = userResolverService;
            _contaUsuarioRepository = contaUsuarioRepository;
            _mapper = mapper;
            _contaUsuarioAppService = contaUsuarioAppService;
            _unitOfWork = unitOfWork;
        }

        public IActionResult Todos()
        {
            var users = _userManager.Users
                            .AsNoTracking()
                            .Include(x => x.ApplicationUserRoles)
                            .ThenInclude(xt => xt.ApplicationRole)
                            .ToList();

            var applicationRoles = _rolesManager.Roles
                                        .AsNoTracking()
                                        .Select(x => x.NormalizedName)
                                        .ToList();

            //Dados cards
            var usuarioCardViewModel = new UsuarioCardViewModel()
            {
                Ativos = users.Count,
                FalhasAcesso = users.Sum(x => x.AccessFailedCount),
                TwoFactors =  users.Where(c => c.TwoFactorEnabled == true).Count(),
                ConfirmacoesPendentes = users.Where(c => c.EmailConfirmed == false).Count(),
                ApplicationRoles = applicationRoles
            };

            return View(usuarioCardViewModel);
        }

        public IActionResult Perfil()
        {
            var currentUserId = _userResolverService.GetUserId();

ReloadCurrenteUser:
            var currentUser = _userManager.Users
                                .Include(x => x.ContaUsuario)
                                .FirstOrDefault(x => x.Id == currentUserId.ToString());

            
            if (currentUser.ContaUsuario == null)
            {
                var contaUsuario = new ContaUsuario();
                contaUsuario.Nome = currentUser.Email;
                contaUsuario.Setor = SetorEnum.NAO_INFORMADO;
                contaUsuario.Funcao = FuncaoEnum.NAO_INFORMADO;
                contaUsuario.PerfilFoto = "data:image/png;base64,";

                _contaUsuarioRepository.Add(contaUsuario);

                goto ReloadCurrenteUser;
            }

            var currentUserMapped = _mapper.Map<ApplicationUserViewModel>(currentUser);

            currentUserMapped.Setores = EnumExtensions<SetorEnum>.GetNames().ToList();
            currentUserMapped.Funcoes = EnumExtensions<FuncaoEnum>.GetNames().ToList();

            return View(currentUserMapped);
        }

        [HttpPost]
        public async Task<IActionResult> UpdatePerfil([FromBody]
                                                      ApplicationUserViewModel applicationUserViewModel)
        {
            //obtenho o id depois o usuario
            var currentUserId = _userResolverService.GetUserId();
            var user = await _userManager.FindByIdAsync(currentUserId.ToString());
            
            //reset da senha, caso tenha vindo uma senha
            if (!applicationUserViewModel.Password.IsNullOrEmpty())
            {
                //token para reset da senha
                var token = await _userManager.GeneratePasswordResetTokenAsync(user);

                var changePassword = await _userManager.ResetPasswordAsync(user, token, applicationUserViewModel.Password);

                if (!changePassword.Succeeded)
                {
                    return BadRequest(changePassword);
                }
            }
            
            //update no usuario e conta do usuario
            var contaUsuarioDB = _contaUsuarioRepository.GetByUserIdNotIncludes(currentUserId.ToString());
            
            if (contaUsuarioDB == null)
                return BadRequest("Usuário não possui uma conta registrada. Informe o usuário MASTER para que seja cadastrada uma conta de usuário para o usuário atual.");
            
            contaUsuarioDB.Nome = applicationUserViewModel.Nome;
            contaUsuarioDB.Setor = (SetorEnum)EnumExtensions<SetorEnum>.GetNumberByStringName(applicationUserViewModel.Setor);
            contaUsuarioDB.Funcao = (FuncaoEnum)EnumExtensions<FuncaoEnum>.GetNumberByStringName(applicationUserViewModel.Funcao);
            contaUsuarioDB.PerfilFoto = applicationUserViewModel.PerfilFoto
                                    .IsNullOrEmpty() ? 
                                    contaUsuarioDB.PerfilFoto : applicationUserViewModel.PerfilFoto;
            contaUsuarioDB.TemaAtual = applicationUserViewModel.ContaUsuarioViewModel.TemaAtual;

            //update user first
            _contaUsuarioRepository.Update(contaUsuarioDB);
            _unitOfWork.Commit();

            return Ok(applicationUserViewModel);
        }

        [HttpGet]
        public IActionResult SearchFuncao(string term)
        {
            if(!string.IsNullOrEmpty(term))
            {
                var funcoes = EnumExtensions<FuncaoEnum>.GetNames().ToList();
                var data = funcoes.Where(a => a.Contains(term, StringComparison.OrdinalIgnoreCase)).ToList();
                var funcoesMapped = new List<GenericSelect2ViewModel>();

                foreach (var item in data.ToList())
                {
                    var tmp = new GenericSelect2ViewModel()
                    {
                        Id = EnumExtensions<FuncaoEnum>.GetNumberByStringName(item),
                        Titulo = item.ToString()
                    };

                    funcoesMapped.Add(tmp);
                }

                return Ok(funcoesMapped);

            }
            else
            {
                return Ok();
            }
        }

        [HttpGet]
        public IActionResult SearchSetor(string term)
        {
            if(!string.IsNullOrEmpty(term))
            {
                var setores = EnumExtensions<SetorEnum>.GetNames().ToList();
                var data = setores.Where(a => a.Contains(term, StringComparison.OrdinalIgnoreCase)).ToList();
                var setoresMapped = new List<GenericSelect2ViewModel>();

                foreach (var item in data.ToList())
                {
                    var tmp = new GenericSelect2ViewModel()
                    {
                        Id = EnumExtensions<SetorEnum>.GetNumberByStringName(item),
                        Titulo = item.ToString()
                    };

                    setoresMapped.Add(tmp);
                }

                return Ok(setoresMapped);
            }
            else
            {
                return Ok();
            }
        }

        public IActionResult GetCurrentUser()
        {
            var currentUserId = _userResolverService.GetUserId();
            var currentUser = _userManager.Users
                                .Include(x => x.ContaUsuario)
                                .Include(x => x.Tenant)
                                .FirstOrDefault(x => x.Id == currentUserId.ToString());
            
            var currentUserMapped = _mapper.Map<ApplicationUserViewModel>(currentUser);
            
            currentUserMapped.ContaUsuarioViewModel.Setor = EnumExtensions<SetorEnum>.GetDisplayValue(currentUser.ContaUsuario.Setor);

            currentUserMapped.ContaUsuarioViewModel.Funcao = EnumExtensions<FuncaoEnum>.GetDisplayValue(currentUser.ContaUsuario.Funcao);

            currentUserMapped.ContaUsuarioViewModel.TemaAtual = currentUser.ContaUsuario.TemaAtual;

            return Ok(new { currentUserMapped });
        }
    }
}