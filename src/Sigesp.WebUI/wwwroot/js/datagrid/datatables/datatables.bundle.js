			...
			_openEditModal: function () {
				...
				for (var i in dt.context[0].aoColumns) {
					...
					columnDefs[i] = {
						...
						editorOnChange: (obj.editorOnChange ? obj.editorOnChange : false),
						editorOnBlur: (obj.editorOnBlur ? obj.editorOnBlur : false)
					};
				}
				var adata = dt.rows({
					selected: true
				...
				for (var j in columnDefs) {
					...
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
						...
						data += "<div class='form-group row' id='"+ 'row_' + columnDefs[j].name +"'>"
				...
				for (var j in columnDefs) {
					...
					// custom onblur triggers
                    if (columnDefs[j].editorOnBlur) {
                        // $.escapeSelector requires jQuery 3.x
                        $(selector).find("#" + $.escapeSelector(columnDefs[j].name)).attr('alt-editor-id', this._quoteattr(j));
                        $(selector).find("#" + $.escapeSelector(columnDefs[j].name)).on('blur', function(elm) {
                            var f = columnDefs[$(this).attr('alt-editor-id')].editorOnBlur;
                            f(elm, that);
                        });
                    }
				}
